...
%Az előbbi kommentemben felett kérdést végülis itt magyarázod, szóval ez így okés.
%Azért bennhagyom, mert egy bíráló fejében is ilyesmik lesznek majd, ezt érdemes úgy kb tudni, és ennek fényében esetleg újragondolni ezt-azt
...
\section{A transformations csomag}
...
Erre a \ref{src:TypeVisitor}. forráskódban láthatunk példát.
...
Egy átalakítás, működése (magas szinten) % #kékestető #1014m #amúgyezjóakkor
...
%akkor node=részfa? ezt gondold végig, hogy mit jelent a node, csúcs, részfa, és hasznád kínosan következetesen ezeket a szavakat (akkor is, ha szóismétlés van)
%Válasz: yes, mivel bizti az ast.AST-ből származik, de ezt inkább kihagyom innen
...
Az \emph{In-place} szabályok az AST egy node-ját alakítják át,
...
Minden \emph{SimpleRule} osztályból származó szabály rendelkezik egy valószínűségi változóval
...
\begin{center}
	\begin{longtable}{ | p{0.25\textwidth} | p{0.6\textwidth} | p{0.05\textwidth} | }
		...
		\emph{GuardDef}
		& \textbf{else} ág \textbf{return} utasítást korai \textbf{return} utasítássá alakító szabály
		& 1.0
		\\ \hline
		...
		\emph{InsertPass}
		& \textbf{pass} utasítás beszúrása egy függvénydefinícióba
		& 0.11
		\\ \hline
...
A leggyakoribb szabály például az \emph{InsertPass} volt, 1000 kódon 771-szer lett alkalmazva,
...
%asszem amúgy list comprehension magyarul listakifejezés <- itt ezt inkább nem használnám, mert félrevezető lehet, a comprehension egy külön node típus a nyelvtanban
...
A "truthy" értékek olyan literálok vagy objektumok amiket boolean-né alakítva igazat kapunk.
...
és a megjelölt \emph{For} node-okat a generált kifejezésre változtatjuk.
...
A \emph{Code} a forráskódok modellje, a \emph{CodeChange} pedig a forráskód-párokat modellezi.
...
az adatelérési modellek (lásd \ref{sec:model}. alfejezet),
...
Ha a nézet valamelyik komponensét az állapotmodell egy változásának hatására szeretnénk frissíteni,
...
Az \emph{AppRules} a szabályok állapotát modellezi, szintén az \emph{Observable}-ből
származik.
...
A vezérlő használja a modellt, lekérdezéseket vagy változtatásokat végez rajta,
...
Az ekvivalens szabályok által végzett átalakítások nem ronthatnak el egy kódot sem,
...
A tesztek futtatásához szükségünk van a \emph{QuixBugs} szoftvertárolóra, % ez a repo elvileg
...
A függvény először a cache-eli az összes helyes megoldást tartalmazó forráskódot,
